# osx specific settings, too lazy to explain what they are for, whatever is copy pasted will be explained.

echo "Apply some macos controlled settings at least..."
...
# add some metadata to some folders to nexer index
#find . -type d -path './.*' -prune -o -path './Pictures*' -prune -o -path './Library*' -exec touch '{}/.metadata_never_index' \; -print
...
systemsetup -settimezone "Europe/Sofia" > /dev/null

# Stop iTunes from responding to the keyboard media keys
launchctl unload -w /System/Library/LaunchAgents/com.apple.rcd.plist 2> /dev/null

# Set computer name
export MAC_MODEL=`sysctl hw.model | awk '{print substr($2, 1, length($2)-2)}'`
sudo scutil --set ComputerName "boz.$MAC_MODEL.bozhev.net"
sudo scutil --set LocalHostName "boz.$MAC_MODEL.bozhev.net"
sudo scutil --set HostName "boz.$MAC_MODEL.bozhev.net"
sudo defaults write /Library/Preferences/SystemConfiguration/com.apple.smb.server NetBIOSName -string "boz.$MAC_MODEL.bozhev.net"

# Disable hibernation for the begining, after that fine tune the shit you want, this script can't be and won't be that specific.

# re-enable via - sudo pmset -a hibernatemode 3
# use this if you want to not-hibernate if file created or whatnot fancy option: sudo pmset -a standby 0
